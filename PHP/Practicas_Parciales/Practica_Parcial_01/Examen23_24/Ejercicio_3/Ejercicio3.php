<?php
    ...
    $ad = new AccesoDatos('estancias.dat');
...
<body>
    ...
    <?php
        if(isset($_POST['crear'])){
            ...
            // Creamos una estancia y la insertamos en el fichero
            ...
            if(isset($_POST['camaSupletoria'])){
                $camaSupletoria = true;
            }else{
                $camaSupletoria = false;
            }

            if(isset($_POST['lavanderia'])){
                $lavanderia = true;
            }else{
                $lavanderia = false;
            }

            $e = new Estancia($dni,$nombreCliente,$_POST['tipoHabitacion'],$numNoches,$_POST['estancia'],$tarjeta,$efectivo,$cuna,$camaSupletoria,$lavanderia);
            $ad->insertarEstancia($e);
        }

        if(isset($_POST['ver'])){
            $estancias = $ad->obtenerEstancia();
            echo '<h3>Estancias</h3>';
            echo '<table border="1px">';
            echo '<tr><th>DNI</th><th>Nombre Cliente</th><th>Tipo Habitacion</th><th>Numero Noches</th><th>Estancia</th><th>Tarjeta</th><th>Efectivo</th><th>Cuna</th><th>Cama Supletoria</th><th>Lavanderia</th></tr>';
            foreach ($estancias as $e) {
                echo '<tr>';
                echo '<td>' . $e->getDni() . '</td>';
                echo '<td>' . $e->getNombreCliente() . '</td>';
                echo '<td>' . $e->getTipoHabitacion() . '</td>';
                echo '<td>' . $e->getNumNoches() . '</td>';
                echo '<td>' . $e->getTipoEstancia() . '</td>';
                echo '<td>' . $e->getTarjeta() . '</td>';
                echo '<td>' . $e->getEfectivo() . '</td>';
                echo '<td>' . $e->getCuna() . '</td>';
                echo '<td>' . $e->getCamaSupletoria() . '</td>';
                echo '<td>' . $e->getLavanderia() . '</td>';
                echo '</tr>';
            }
            echo '</table>';
        }
